...
module.exports = function (app, songsRepository, usersRepository) {
    ...
    app.delete('/api/v1.0/songs/:id', function (req, res) {
        try {
            let songId = new ObjectId(req.params.id)
            let filter = {_id: songId}
            isTheAuthorOf(res.user, songId).then(isAuthor => {
                if (isAuthor) {
    ...
    app.put('/api/v1.0/songs/:id', function (req, res) {
        try {
            let songId = new ObjectId(req.params.id);
            let filter = { _id: songId };
            // Si la _id no existe, no se crea un nuevo documento.
            ...
            isTheAuthorOf(song.author, songId).then(isAuthor => {
                if (isAuthor) {
                    validatorInsertSong(song, function (errors) {
                        if (errors !== null && errors.length > 0) {
                            res.status(409);
    ...
    async function isTheAuthorOf(user, songId) {
